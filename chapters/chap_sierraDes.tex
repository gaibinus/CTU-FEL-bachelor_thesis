...
\midinsert
    ...
    \caption/f 3D render of the Board Sierra from the top (left) and the bottom (right) side. A real scaled version of this visualization is available in figures \ref[app_vis:sierraTop] and \ref[app_vis:sierraBottom]. 
\endinsert
...
\midinsert \clabel[tab:subsystemConsumption]{Subsystems consumption}
    \ctable{lcl}{
        Subsystem           & Typ. current $\rm{[mA]}$  & Note \crl
        Processing unit     & \phantom{0}20 @ 3.3[V] & at $80\rm{[MHz]}$ clock \cr
        Power management    & 0.3 @ \phantom{3.}-[V] & for each $3.3\rm{[V]}$ and $5\rm{[V]}$ bus \cr 
