...
\midinsert
    ...
    \caption/f Photos of existing professionally developed \glref{OBC}s. iOBC by ISIS (left) and SatBus 3C2 by NanoAvionics (right). Sources: ISIS, NanoAvionics.
\endinsert
...
\quad Since the VST104 project is mainly about designing electronics hardware, proper \glref{PCB} design software had to be selected. There are multiple professional software packages available, such as Altium Designer or CadSoft EAGLE. Although these design tools come with powerful features, they do not fit with the open-source policy of the project (section \ref[chap:openSource]). The \glref{VST} supervisors chose the KiCad EDA instead. This tool is probably the most popular one in the open hardware and makers community. On top of that, both LibreCube and TUDSaT use KiCad for their projects.
